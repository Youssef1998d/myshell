...
class CMD:
    ...
    def __init__(self, shell_mode: Mode = Mode.without_file, file_input: str = None):
        self.shell_mode = shell_mode
        self.current_directory = os.getcwd()
        self.current_user = os.getenv('USER')
        self.current_host = "pythonMysh"
        self.set_promp()
        if file_input is None and shell_mode == Mode.with_file:
            raise ValueError("file mandatory for usage!")
    ...
    def run(self):
        if self.shell_mode == Mode.without_file:
